...
// Parameterized "What's that car?" → scan → result spot (13s = 390 frames).
// House music baked in.

export type ScanSpotProps = {
  category: string
  rarity: number
  year: string
  make: string
...
function Scan() {
  ...
  const corner = (pos: React.CSSProperties): React.CSSProperties => ({
    position: 'absolute', width: 90, height: 90, borderColor: CYAN, boxShadow: `0 0 24px ${CYAN}66`, ...pos,
  })
  return (
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', opacity }}>
      <div style={{ position: 'relative', width: 760, height: 520 }}>
        <svg viewBox="0 0 200 110" width="640" height="352" style={{ position: 'absolute', left: 60, top: 90, opacity: 0.92 }}>
          <path d="M10 78 Q14 60 34 56 L60 40 Q78 30 110 30 L150 32 Q172 36 184 56 L190 60 Q196 64 196 74 L196 82 Q196 88 188 88 L172 88 A14 14 0 0 0 144 88 L70 88 A14 14 0 0 0 42 88 L18 88 Q10 88 10 82 Z" fill="url(#g)" stroke={CYAN} strokeWidth="1.4" />
          <defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stopColor="#13344a" /><stop offset="1" stopColor="#1b1330" /></linearGradient></defs>
...
function Result({ p }: { p: ScanSpotProps }) {
  const frame = useCurrentFrame()
  const opacity = fadeInOut(frame, 185, 305, 12, 10)
  const y = slideUp(frame, 185, 22, 80)
  const sc = scaleIn(frame, 185, 22, 0.92)
  const lo = Math.round(interpolate(frame, [200, 250], [0, p.valueLo], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }))
  const hi = Math.round(interpolate(frame, [200, 250], [0, p.valueHi], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }))
  return (
    <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', opacity }}>
      <div style={{ width: 860, background: PANEL, border: `2px solid ${LINE}`, borderRadius: 40, padding: 56, transform: `translateY(${y}px) scale(${sc})`, boxShadow: `0 30px 90px rgba(0,0,0,0.6), 0 0 120px rgba(34,211,238,0.10)` }}>
        <div style={{ display: 'flex', gap: 16, marginBottom: 28 }}>
          <Chip label={p.category} />
          <Chip label={`★ Rarity ${p.rarity}/10`} color={VIOLET} bg="rgba(168,85,247,0.10)" />
        </div>
        <div style={{ fontSize: 34, color: MUTE, fontWeight: 700 }}>{p.year}</div>
        <div style={{ fontSize: 76, fontWeight: 800, color: '#fff', letterSpacing: '-0.02em', lineHeight: 1.02 }}>{p.make}</div>
        <div style={{ fontSize: 76, fontWeight: 800, color: '#fff', letterSpacing: '-0.02em', lineHeight: 1.02, marginBottom: 28 }}>{p.model}</div>
